...
macro_rules! smallvec {
    ...
    ($($x:expr),*$(,)*) => ({
        let count = 0_usize $(+ $crate::smallvec!(@one $x))*;
        #[allow(unused_mut)]
        ...
        if count <= vec.inline_size() {
            ...
            $crate::SmallVec::from_vec(::std::vec![$($x,)*])
        }
    });
}
...
fn layout_array<T>(n: usize) -> Result<Layout, CollectionAllocErr> {
    ...
    // TODO: investigate `Layout::array::<T>`, instead of
    // `Layout::from_size_align`
    Layout::from_size_align(size, align)
        .map_err(|_| CollectionAllocErr::CapacityOverflow)
    // Layout::array::<T>(size).map_err(|_|
...
impl<'a, T: 'a + Array> ExactSizeIterator for Drain<'a, T> {
    #[inline]
    fn len(&self) -> usize {
        self.iter.len()
    }
...
impl<A: Array> SmallVec<A> {
    ...
    #[must_use]
    #[inline]
    pub fn from_vec(mut vec: Vec<A::Item>) -> Self {
        if vec.capacity() <= Self::inline_capacity() {
            unsafe {
                let mut data =
                    SmallVecData::<A>::from_inline(MaybeUninit::uninit());
                let len = vec.len();
                vec.set_len(0);
    ...
    #[inline]
    pub fn from_buf_and_len(buf: A, len: usize) -> Self {
        assert!(len <= A::size());
        unsafe {
            SmallVec::from_buf_and_len_unchecked(MaybeUninit::new(buf), len)
        }
    }
        ...
        R: RangeBounds<usize>,
    {
        ...
        // SAFETY: We asserted start and end lengths are in bounds, so settine
        // new length is safe.
        unsafe {
            self.set_len(start);
    ...
    /// `A` contains uninitialized elements), or if the `SmallVec` is too long
    /// (and all the elements were spilled to the heap).
    ...
    /// calling the closure `f`. The return values from `f`
    ...
    pub fn as_slice(&self) -> &[A::Item] {
        ...
        unsafe {
            core::slice::from_raw_parts(
                self.data.as_ptr().add(self.current),
                len,
            )
        }
    }
...
impl<A: Array> IntoIterator for SmallVec<A> {
    ...
    fn into_iter(mut self) -> Self::IntoIter {
        ...
        // SAFETY: Set SmallVec len to zero as `IntoIter` drop handles dropping
        // of the elements
...
// unsafe impl<T, const N: usize> Array for bitarr!(for N, in Lsb0, T) {
//     type Item = T;
...
// impl<A: Array> ToSmallVec<A> for BitVec
// where
//     A::Item: Copy,
//     A::Item: Into<usize>,
// {
